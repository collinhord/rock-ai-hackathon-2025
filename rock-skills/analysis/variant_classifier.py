...
def identify_grade_progressions(skills_df: pd.DataFrame) -> dict:
    ...
    for authority in normalized['AUTHORITY'].unique():
        ...
        for idx1, skill1 in authority_skills.iterrows():
            ...
            chain = [(skill1['SKILL_ID'], skill1['GRADE_LEVEL_SHORT_NAME'])]
            processed.add(skill1['SKILL_ID'])
            ...
            for idx2, skill2 in authority_skills.iterrows():
                ...
                if skill1['GRADE_LEVEL_NAME'] != skill2['GRADE_LEVEL_NAME']:
                    ...
                    if similarity >= SIMILARITY_THRESHOLD:
                        chain.append((skill2['SKILL_ID'], skill2['GRADE_LEVEL_SHORT_NAME']))
                        processed.add(skill2['SKILL_ID'])
            
            # Only keep chains with progression across grades
            if len(chain) >= MIN_GROUP_SIZE:
                # Sort by grade level
                chain.sort(key=lambda x: GRADE_ORDER.index(x[1]) if x[1] in GRADE_ORDER else 999)
                
                for idx, (skill_id, _) in enumerate(chain):
                    grade_chains[skill_id] = (f"SB-{chain_id:04d}", idx + 1)
                chain_id += 1
...
def main():
    ...
    grade_chains = identify_grade_progressions(skills_df)
