...
│   ├── variant_classifier.py               # State A/B classification (NEW)
│   ├── llm_mapping_assistant.py            # LLM-assisted taxonomy mapping (NEW)
│   ├── batch_map_skills.py                 # Batch mapping pipeline (NEW)
...
│   ├── variant-classification-report.csv   # State A/B classifications (NEW)
│   ├── README.md                           # Analysis tools documentation (NEW)
...
### 1b. Run Variant Classification (State A vs State B) 🆕

```bash
cd analysis

# Classify all 8,224 skills into State A (cross-state) vs State B (grade progressions)
python3 variant_classifier.py
```

**Runtime**: ~15-20 minutes  
**Outputs**:
- `variant-classification-report.csv` - All skills with State A/B classifications (2.6MB)
- `variant-classification-report_summary.csv` - Group summary (407 groups)

**Results**:
- **187 State A groups** (383 skills) - Cross-state variants for content tagging
- **220 State B chains** (569 skills) - Grade progressions for learning paths
- **7,272 unique skills** (88%) - Single-state or specialized skills

### 1c. Test LLM-Assisted Mapping 🆕

```bash
cd analysis

# Test LLM mapping on 3 sample ELA skills (requires AWS credentials)
...
- Hybrid: Embedding retrieval + LLM ranking
- Confidence scoring (High/Medium/Low)
- Alternative suggestions

See `analysis/QUICKSTART.md` for detailed examples and batch processing.
...
### Phase 5: Discovery Expansion ✅ NEW
- [x] State A vs State B variant classification system
- [x] Automated classification of all 8,224 skills
- [x] LLM-assisted taxonomy mapping (AWS Bedrock integration)
- [x] Batch mapping pipeline with checkpoint/resume
- [x] Updated problem statement with State A/B distinction
- [x] **Result**: 187 State A groups, 220 State B chains identified; LLM acceleration ready

## Key Findings

### Problem Quantified
- **8,224 total ROCK skills** analyzed (actual count from classification)
...
### New Discovery: State A vs State B 🆕
- **State A (Cross-State Variants)**: 187 groups, 383 skills (~5%)
  - Same concept, different states, same grade
  - Solution: Enable "tag once, applies everywhere" for P&I
- **State B (Grade Progressions)**: 220 chains, 569 skills (~7%)
  - Related concepts spiraling across grades
  - Solution: Prerequisite chains for learning paths
- **Unique Skills**: 7,272 skills (88%)
  - Specialized or single-state skills
  - Solution: Standard 1:1 taxonomy mapping
...
- **LLM-assisted mapping** accelerates taxonomy alignment
- **Automated classification** identifies State A/B patterns
- **Interactive POC** shows immediate value

### Value Proposition
- **70-80% reduction** in skill discovery time
- **Enables new features**: adaptive learning, cross-state alignment, prerequisite chains
- **Research-grade data**: aggregate by master concepts
- **Competitive advantage**: learning science leadership
- **Scalable approach**: LLM assists with 2,000+ skill mapping effort
...
- **sklearn**: TF-IDF, cosine similarity, semantic analysis
- **sentence-transformers**: Embedding models for semantic search
- **AWS Bedrock**: Claude Sonnet 4.5 for LLM-assisted mapping 🆕
